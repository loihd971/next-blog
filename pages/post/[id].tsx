...
export default function Post({ relatedPost }: Props | any) {
  ...
  const getPostDetail = async () => {
    try {
      const res = await axios.get(`https://evanloi991.vercel.app/api/post/${id}`);
      setPost(res.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleFollowUser = async (type: string) => {
    ...
    try {
      await axios.post(`https://evanloi991.vercel.app/api/user/follow`, body);
      getAuthor();
    } catch (error) {
      console.log(error);
    }
  ...
  const getCommentList = async () => {
    try {
      const res: any = await axios.get(
        `https://evanloi991.vercel.app/api/comment/${post?._id}`
      );
  ...
  const handleReplyComment = async (parentId: string | null, value: string) => {
    ...
    try {
      const res: any = await axios.post(
        `https://evanloi991.vercel.app/api/comment`,
        body
      );
  ...
  const handleEditComment = async (id: string | null, value: string) => {
    ...
    try {
      const res: any = await axios.put(
        `https://evanloi991.vercel.app/api/comment/${id}`,
        body
      );
  ...
  const handleLikePost = async () => {
    try {
      await axios.post(`https://evanloi991.vercel.app/api/post/${post?._id}`, {
        userId: session?.user?.id,
      });
      getPostDetail();
      setIsCommented(Math.random());
  ...
  const handleLikeComment = async (id: string | null) => {
    try {
      await axios.post(`https://evanloi991.vercel.app/api/comment/${id}`, {
        type: "like",
        userId: session?.user?.id,
      });
    ...
    if (post?.userId) {
      try {
        const res = await axios.get(
          `https://evanloi991.vercel.app/api/user/${post?.userId}`
        );
        setAuthor(res.data);
      } catch (error: any) {
        console.log(error);
...
export const getStaticProps = async ({ params }: any) => {
  try {
    const posts = await axios.get(
      `https://evanloi991.vercel.app/api/post/related-post`,
      {
        params: {
          postId: params.id,
        },
